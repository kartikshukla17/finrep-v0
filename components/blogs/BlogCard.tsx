...
export default function BlogCard({ slug, title, date, image, className = '' }: BlogCardProps) {
    return (
        <Link
            href={`/blogs/${slug}`}
            className={`flex flex-col justify-start items-start gap-6 hover:opacity-90 transition-opacity ${className}`}
        >
